              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="p-4 space-y-2.5">
                    {[
                      { l: "Procurement", v: fmtCurrency(demoResult.procPerKg, currency, fxRate) },
                      { l: "Freight (FOR)", v: fmtCurrency(demoResult.freightPerKg, currency, fxRate) },
                      { l: "Packaging", v: fmtCurrency(demoResult.packPerKg, currency, fxRate) },
                    ].map(({ l, v }) => (
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-mono text-[13px] font-bold text-slate-800">{fmtCurrency(demoResult.subtotalPerKg, currency, fxRate)}/kg</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[12.5px] text-emerald-600 font-semibold">Net Margin</span>
                      <span className="font-mono text-[13px] font-bold text-emerald-500">{fmtCurrency(demoResult.marginPerKg, currency, fxRate)}/kg</span>
                    </div>
                    <div className="mt-2 pt-2 border-t-2 border-slate-800 flex justify-between items-center">
                      ...
                      <span className="font-mono text-[17px] font-bold text-slate-900">{fmtCurrency(demoResult.sellingPerKg, currency, fxRate)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[11px] text-slate-400">Grand Total</span>
                      <span className="font-mono text-[12px] font-bold text-slate-500">{fmtCurrencyShort(demoResult.grandTotal, currency, fxRate)}</span>
                    </div>
                  </div>
                </div>
                ...
                <motion.div
                  ...
                  className="rounded-xl border-2 overflow-hidden bg-white hover:shadow-md transition-all"
                >
                  ...
                  <div className={`p-4 space-y-2.5 ${!hasData ? "opacity-40" : ""}`}>
                    {[
                      { l: "Procurement", v: fmtCurrency(result.procPerKg, currency, fxRate) },
                      { l: `Freight (${inp.freightMode})`, v: fmtCurrency(result.freightPerKg, currency, fxRate) },
                      { l: inp.packagingEnabled ? "Packaging" : "Packaging (off)", v: inp.packagingEnabled ? fmtCurrency(result.packPerKg, currency, fxRate) : "₹0.00", muted: !inp.packagingEnabled },
                    ].map(({ l, v, muted }) => (
                      <div key={l} className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-[12.5px] font-bold text-slate-700">Cost Stack</span>
                      <span className="font-mono text-[13px] font-bold text-slate-800">{fmtCurrency(result.subtotalPerKg, currency, fxRate)}/kg</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className={`text-[12.5px] font-semibold ${result.exceedsCap && hasData ? "text-red-500" : "text-emerald-600"}`}>Net Margin</span>
                      <span className={`font-mono text-[13px] font-bold ${result.exceedsCap && hasData ? "text-red-500" : "text-emerald-500"}`}>{fmtCurrency(result.marginPerKg, currency, fxRate)}/kg</span>
                    </div>
                    <div className={`mt-2 pt-2 border-t-2 flex justify-between items-center ${result.exceedsCap && hasData ? "border-red-400" : "border-slate-800"}`}>
                      ...
                      <span className={`font-mono text-[17px] font-bold ${result.exceedsCap && hasData ? "text-red-600" : "text-slate-900"}`}>
                        <AnimatedNumber value={result.sellingPerKg / fxRate} prefix={currency === "INR" ? "₹" : currency === "USD" ? "$" : "€"} />
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[11px] text-slate-400">Grand Total</span>
